...
// =============================================================================
const TOOL_CALL_SIGNATURE_PATTERNS = [
    ...
    return {
        ...
        classifyAction(params, _targets) {
            ...
            if (params.command || params.cmd)
                return 'shell_exec';
            if (params.old_string || params.search || params.patch)
                return 'file_edit';
            if (params.content && (params.file || params.file_path || params.path))
                return 'file_create';
            if (params.url || params.endpoint)
                return 'api_call';
            if (params.query || params.pattern || params.glob)
                return 'search';
            return undefined;
        },
        extractParameters(raw) {
            ...
            if (raw.actionClass)
                extracted.actionClass = raw.actionClass;
 ...
 */
export function toolCallToProposal(toolName, args) {
 ...
 */
export function toolCallToOutcome(toolName, args, result) {
