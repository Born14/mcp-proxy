...
export class ConstraintStore {
     ...
     */
    seedFromOutcome(outcome, sessionId) {
        // Rule 1: Infrastructure faults don't seed
        if (outcome.failureKind === 'harness_fault')
            return null;
        ...
        const sessionConstraints = sessionId
            ...
            : this.constraints;
        if (sessionConstraints.length >= this.config.maxConstraintDepth)
            return null;
        ...
        if (!constraint)
            return null;
        // Dedup
        if (this.isDuplicate(constraint))
            return null;
    ...
    gc() {
        ...
        this.constraints = this.constraints.filter(c => !c.expiresAt || c.expiresAt > now);
    ...
    // CONSTRAINT BUILDERS
    // =========================================================================
    buildStrategyBan(outcome, occurrences) {
        return {
    ...
    buildParameterBan(outcome, occurrences) {
    ...
    checkStrategyBan(constraint, proposal) {
        if (!constraint.actionClass)
            return null;
    ...
    checkRadiusLimit(constraint, proposal) {
        if (constraint.maxChanges === undefined)
            return null;
    ...
    checkParameterBan(constraint, proposal) {
        if (!constraint.bannedParameter || !constraint.bannedValues)
            return null;
    ...
    isDuplicate(candidate) {
        return this.constraints.some(c => c.type === candidate.type &&
